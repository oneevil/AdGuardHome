...
type jsonDNSConfig struct {
	...
	// MikroTik contains configuration for MikroTik REST API integration.
	MikroTik *jsonMikroTikConfig `json:"mikrotik"`
}
...
// jsonMikroTikConfig contains configuration for MikroTik REST API integration.
type jsonMikroTikConfig struct {
	// URL is the base URL of the MikroTik router REST API.
	URL string `json:"url"`
	...
	// IPv6, if true, also adds IPv6 addresses to MikroTik IPv6
	// firewall address-list.
	IPv6 bool `json:"ipv6"`

	// UseDNSTTL, if true, uses the DNS response TTL as the timeout
	// for address-list entries instead of the fixed Timeout value.
	UseDNSTTL bool `json:"use_dns_ttl"`
}
...
func (s *Server) getDNSConfig(ctx context.Context) (c *jsonDNSConfig) {
	...
	var mikrotikConf *jsonMikroTikConfig
	if s.conf.MikroTik != nil {
		mikrotikConf = &jsonMikroTikConfig{
			URL:       s.conf.MikroTik.URL,
			Username:  s.conf.MikroTik.Username,
			Password:  s.conf.MikroTik.Password,
			Insecure:  s.conf.MikroTik.Insecure,
			Timeout:   s.conf.MikroTik.Timeout,
			IPv6:      s.conf.MikroTik.IPv6,
			UseDNSTTL: s.conf.MikroTik.UseDNSTTL,
		}
	}
...
func (s *Server) setConfigRestartable(dc *jsonDNSConfig) (shouldRestart bool) {
	...
	// Handle MikroTik configuration
	if dc.MikroTik != nil {
		newConf := &ipset.MikroTikConfig{
			URL:       dc.MikroTik.URL,
			Username:  dc.MikroTik.Username,
			Password:  dc.MikroTik.Password,
			Insecure:  dc.MikroTik.Insecure,
			Timeout:   dc.MikroTik.Timeout,
			IPv6:      dc.MikroTik.IPv6,
			UseDNSTTL: dc.MikroTik.UseDNSTTL,
		}

		if s.conf.MikroTik == nil ||
			s.conf.MikroTik.URL != newConf.URL ||
			s.conf.MikroTik.Username != newConf.Username ||
			s.conf.MikroTik.Password != newConf.Password ||
			s.conf.MikroTik.Insecure != newConf.Insecure ||
			s.conf.MikroTik.Timeout != newConf.Timeout ||
			s.conf.MikroTik.IPv6 != newConf.IPv6 ||
			s.conf.MikroTik.UseDNSTTL != newConf.UseDNSTTL {
			s.conf.MikroTik = newConf
			shouldRestart = true
		}
	}
